...
# ------------------------------------------------------------------------------
JID2=$(sbatch --job-name=deval_mono "${SCRIPT}" \
...
# ------------------------------------------------------------------------------
JID3=$(sbatch --job-name=deval_lora_rag "${SCRIPT}" \
...
# ------------------------------------------------------------------------------
JID4=$(sbatch --job-name=deval_pnr "${SCRIPT}" \
...
# ------------------------------------------------------------------------------
JID5=$(sbatch --job-name=deval_xlora "${SCRIPT}" \
...
# ------------------------------------------------------------------------------
JID6=$(sbatch --job-name=deval_recipe "${SCRIPT}" \
...
#    System 5 is seeded with 21,847 CF triples. The router must fire bypass on
#    D_conflict (→ ESR) and stay silent on D_control TriviaQA (→ FR ≈ 0).
# ------------------------------------------------------------------------------
JID7=$(sbatch --job-name=deval_morpheus "${SCRIPT}" \
    --morpheus \
    --morpheus_state_dir "$(pwd)/morpheus_state" \
    --run_name morpheus_deval \
    | awk '{print $NF}')
echo "[7/8] morpheus         → job ${JID7}"
...
# ------------------------------------------------------------------------------
JID8=$(sbatch --job-name=deval_parallel "${SCRIPT}" \
    --parallel \
    --router_state "${ROUTER_STATE}" \
    --similarity_threshold 0.45 \
    --run_name parallel_deval \
    | awk '{print $NF}')
echo "[8/8] parallel_orch    → job ${JID8}"

echo ""
echo "======================================================================"
echo "Submitted 8 jobs. Results:"
